...
HispanicPopGraphic <- HispanicPopforGraphic %>%
  ggplot(aes(PlaceName.fac, value, fill=description.fac, label = comma(value, accuracy = 1))) + 
  geom_bar(stat="identity",
           position = position_dodge(),
           width = .7,
           color="gray50") +
  geom_text(aes(label = comma(value, accuracy = 1)),position=position_dodge(width = .7), vjust = .5, hjust = -.5, size=2.75, family="Asap") +
