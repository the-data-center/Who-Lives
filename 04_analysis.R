...
hwc <- hwcRaw %>%
  ...
  mutate(tothwc2000 = Married15to64_2000 + MaleHH15to64_2000 + FemaleHH15to64_2000 + Married65plus_2000 + MaleHH65plus_2000 + FemaleHH65plus_2000,
         ...
         census2000 = tothwc2000 / TotalHH_2000,
         ...
         moeprop2000 = moeprop(y = TotalHH_2000, moex = tothwc2000MOE, moey = TotalHH_2000MOE, p = census2000),
         significant = stattest(x=census2000,moex = moeprop2000, y=pcthwc,moey = moeprop)) 
...
veh <- vehRaw %>%
  ...
  select(place, placename, vehpct, census2000, significant)
...
mob <- mobRaw %>%
  ...
  mutate(mobabroadpct = TotMovedfromAbroad / Total,
         ...
         abroadSIG = ifelse(placename == "New Orleans Metro Area", "yes", stattest (x=sf2004mobabroadpct, moex = sf2004mobabroadpctMOE, y=mobabroadpct, moey = mobabroadmoeprop)),
         statesSIG = ifelse(placename == "New Orleans Metro Area", "yes", stattest (x=sf2004mobStatespct, moex = sf2004mobStatespctMOE, y=mobStatespct,moey = mobStatesmoeprop)),
         difparishSIG = ifelse(placename == "New Orleans Metro Area", "yes", stattest (x=sf2004difparishpct, moex = sf2004difparishpctMOE, y =difparishpct, moey = difparishmoeprop)),
         withinparishSIG = ifelse(placename == "New Orleans Metro Area", "yes", stattest (x=sf2004withinparishpct, moex = sf2004withinparishpctMOE, y=withinparishpct, moey = withinparishmoeprop)),
         samhouseSIG = ifelse(placename == "New Orleans Metro Area", "yes", stattest (x=sf2004samehousepct, moex = sf2004samehousepctMOE, y=samehousepct, moey = samehousemoeprop)))
...
rentbur <- rentburRaw %>%
  ...
  mutate(#sf2004=c(0.2432,0.2167,0,0.2161,0.2384),#0 for St. tammany missing value ### ****HT commenting out old values, adding ones I pulled from 2004 ACS
         ...
         significant = ifelse(placename == "New Orleans Metro Area", "yes", stattest(x=sf2004, moex = sf2004MOE, y=rentburpct, moey=moeprop)))
...
hobur <- hoburRaw %>%
  ...
  mutate(sf2004 = hoburpct2004,
         sf2004MOE = as.numeric(hoburpct2004MOE), #getting these numbers from line 154 of data-pull.R
         ...
         moeprop = as.numeric(moeprop(y=Total,moex=moexagg,moey=moeyagg,p=hoburpct)),
         significant = ifelse(placename == "New Orleans Metro Area", "yes", stattest(x=sf2004,moex = sf2004MOE, y=hoburpct, moey=moeprop))) 
...
medrent <- medrentRaw %>%
  ...
  mutate(sf2004 = medgrossrent2004 *cpi04,
         ...
         significant = ifelse(placename == "New Orleans Metro Area", "yes", stattest(x=sf2004, moex = sf2004MOE, y=Rent,moey=RentMOE))) 
...
employ_stattest.data <- employ_stattest_race %>%
...
bach.race_stattest <- bach.race_stattest.data%>%
  mutate(sig_wht_blk = stattest(x=pctbach_wht, moex = moeprop_wht, y=pctbach_blk, moey = moeprop_blk),
         ...
         #the formula is (moe/1.65)/value. for United way, we use the threshold .15. 
         cv_all = (moeprop/1.65)/pctbach,
         cv_white = (moeprop_wht/1.65)/pctbach_wht,
         cv_black = (moeprop_blk/1.65)/pctbach_blk,
         cv_hisp = (moeprop_hisp/1.65)/pctbach_hisp,
         cv_asian = (moeprop_asian/1.65)/pctbach_asian
  )

bach_stat_all <- bach.race_stattest %>% select(place, placename, (contains("sig") & contains("all")), contains("pct"), contains("cv")) %>%
  pivot_longer(cols = contains("pct"), names_to = "race", values_to = "val") %>%
  pivot_longer(cols = contains("cv"), names_to = "race2", values_to = "cv") %>% select(-race2) %>%
  pivot_longer(cols = contains("sig"), names_to = "var", values_to = "stat_all") %>% group_by(place, placename) %>%
...
pov_stattest <- povRaw_exp %>%
  ...
  mutate(sig_wht_blk = stattest(x=pctpov_wht, moex = moeprop_wht, y=pctpov_blk, moey = moeprop_blk),
         ...
         #the formula is (moe/1.65)/value. for United way, we use the threshold .15. 
         cv_all = (moeprop/1.65)/pctpov,
         cv_white = (moeprop_wht/1.65)/pctpov_wht,
         cv_black = (moeprop_blk/1.65)/pctpov_blk,
         cv_hisp = (moeprop_hisp/1.65)/pctpov_hisp,
         cv_asian = (moeprop_asian/1.65)/pctpov_asian
  )

pov_stat_all <- pov_stattest %>% select(place, placename, (contains("sig") & contains("all")), contains("pct"), contains("cv")) %>%
  pivot_longer(cols = contains("pct"), names_to = "race", values_to = "val") %>%
  pivot_longer(cols = contains("cv"), names_to = "race2", values_to = "cv") %>% select(-race2) %>%
  pivot_longer(cols = contains("sig"), names_to = "var", values_to = "stat_all") %>% group_by(place, placename) %>%
...
childpov_stattest <- childpov_stattest.data%>%
  mutate(sig_wht_blk = stattest(x=pctBelowChildPov_wht, moex = moeprop_wht, y=pctBelowChildPov_blk, moey = moeprop_blk),
         ...
         #the formula is (moe/1.65)/value. for United way, we use the threshold .15. 
         cv_all = (moeprop/1.65)/pctBelowChildPov,
         cv_white = (moeprop_wht/1.65)/pctBelowChildPov_wht,
         cv_black = (moeprop_blk/1.65)/pctBelowChildPov_blk,
         cv_hisp = (moeprop_hisp/1.65)/pctBelowChildPov_hisp,
         cv_asian = (moeprop_asian/1.65)/pctBelowChildPov_asian
  )
...
ho_stattest <- ho_stattest.data %>%
  mutate(sig_wht_blk = stattest(x=Ownerpct_wht, moex = Ownermoeprop_wht, y=Ownerpct_blk, moey = Ownermoeprop_blk),
         ...
         #the formula is (moe/1.65)/value. for United way, we use the threshold .15. 
         cv = (Ownermoeprop/1.65)/Ownerpct,
         cv_white = (Ownermoeprop_wht/1.65)/Ownerpct_wht,
         cv_black = (Ownermoeprop_blk/1.65)/Ownerpct_blk,
         cv_hisp = (Ownermoeprop_hisp/1.65)/Ownerpct_hisp,
         cv_asian = (Ownermoeprop_asian/1.65)/Ownerpct_asian)
